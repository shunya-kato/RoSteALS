...
from tools.eval_metrics import unormalise, compute_psnr, compute_ssim, compute_mse, compute_lpips, compute_sifid, resize_array, resize_tensor
...
def main(args):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            img_unorm = resize_array(unormalise(img))
            stego_unorm = resize_array(unormalise(stego))
            recon_ae_unorm = resize_array(unormalise(recon_ae))

            # eval stego quality: SSIM, PSNR, MSE, LPIPS
            score_lpips.append(compute_lpips(resize_tensor(img), resize_tensor(stego), lpips_alex))
            score_sifid.append(compute_sifid(resize_tensor(img), resize_tensor(stego), sifid_model))
            ...
            score_lpips_ae.append(compute_lpips(resize_tensor(img), resize_tensor(recon_ae), lpips_alex))
            score_sifid_ae.append(compute_sifid(resize_tensor(img), resize_tensor(recon_ae), sifid_model))
            ...
            score_lpips_recon.append(compute_lpips(resize_tensor(recon_ae), resize_tensor(stego), lpips_alex))
            score_sifid_recon.append(compute_sifid(resize_tensor(recon_ae), resize_tensor(stego), sifid_model))
    ...
    out.update(mse=f"{score_mse.mean():.2f}+-{score_mse.std():.2f}",
    ...
    for i in bit_acc:
        ...
        if i==-1:
            out.update(bit_acc_n0=f"{bit_acc[i].mean():.2f}+-{bit_acc[i].std():.2f}")
    ...
    sample_n5 = (bit_acc_noise > 0.9).mean()
    print(f"sample_n0: {sample_n0:.2f}")
    print(f"sample_n1: {sample_n1:.2f}")
    print(f"sample_n5: {sample_n5:.2f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', "--config", default='models/AE3_s100.yaml', help="Path to config file.")
    parser.add_argument('-w', "--weight", default='/mnt/fast/nobackup/scratch4weeks/tb0035/projects/diffsteg/controlnet/AE_s100_3/checkpoints/epoch=000016.ckpt', help="Path to checkpoint file.")
    ...
    parser.add_argument('-o', 
        "--output", default='/mnt/fast/nobackup/scratch4weeks/tb0035/projects/diffsteg/controlnet/AE_s100_3/results/res.pkl', help="output directory."
    )
    parser.add_argument(
